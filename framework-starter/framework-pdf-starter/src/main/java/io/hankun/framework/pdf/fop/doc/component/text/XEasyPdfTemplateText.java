package io.hankun.framework.pdf.fop.doc.component.text;

import io.hankun.framework.pdf.fop.XEasyPdfTemplateAttributes;
import io.hankun.framework.pdf.fop.XEasyPdfTemplateTags;
...
public class XEasyPdfTemplateText extends XEasyPdfTemplateTextBase {
     ...
     * @return 返回文本组件
     */
    public XEasyPdfTemplateText setWhiteSpaceCollapse(String whiteSpaceCollapse) {
    ...
    private Element createInline(Document document) {
        ...
        // 设置上下左右填充
        Optional.ofNullable(this.param.getPadding()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.PADDING, v.intern().toLowerCase()));
        // 设置上填充
        Optional.ofNullable(this.param.getPaddingTop()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.PADDING_TOP, v.intern().toLowerCase()));
        // 设置下填充
        Optional.ofNullable(this.param.getPaddingBottom()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.PADDING_BOTTOM, v.intern().toLowerCase()));
        // 设置左填充
        Optional.ofNullable(this.param.getPaddingLeft()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.PADDING_LEFT, v.intern().toLowerCase()));
        // 设置右填充
        Optional.ofNullable(this.param.getPaddingRight()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.PADDING_RIGHT, v.intern().toLowerCase()));
        // 设置文本缩进
        Optional.ofNullable(this.param.getTextIndent()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.TEXT_INDENT, v.intern().toLowerCase()));
        // 设置文本语言
        ...
        // 设置字体颜色
        Optional.ofNullable(this.param.getColor()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.COLOR, v.intern().toLowerCase()));
        // 设置垂直对齐
        ...
        // 设置背景颜色
        Optional.ofNullable(this.param.getBackgroundColor()).ifPresent(v -> inline.setAttribute(XEasyPdfTemplateAttributes.BACKGROUND_COLOR, v.intern().toLowerCase()));
        // 设置文本
