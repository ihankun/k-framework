...
public class UnicodeInputStream extends InputStream {
    ...
    protected String readEncoding() throws IOException {
        ...
        if (this.readByte()) {
            switch(this.buf[0]) {
                case -17:
                    encoding = this.match(UTF8_BOM, (UnicodeBOM)null);
                    break;
                case -2:
                    encoding = this.match(UTF16BE_BOM, (UnicodeBOM)null);
                    break;
                case -1:
                    encoding = this.match(UTF16LE_BOM, (UnicodeBOM)null);
                    if (encoding != null) {
                        encoding = this.match(UTF32LE_BOM, encoding);
                    }
                    break;
                case 0:
                    encoding = this.match(UTF32BE_BOM, (UnicodeBOM)null);
                    break;
                default:
                    encoding = null;
            }
