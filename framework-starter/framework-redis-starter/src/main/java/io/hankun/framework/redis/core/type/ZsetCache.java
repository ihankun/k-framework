...
public interface ZsetCache<V> {
     ...
     * 指定分数范围内的所有元素及其分数
     */
    Set<ZSetOperations.TypedTuple<V>> rangeByScoreWithScores(CacheKey cacheKey, double min, double max);

    /**
     * 分页查询 指定分数范围内的所有元素及其分数
     */
    Set<ZSetOperations.TypedTuple<V>> rangeByScoreWithScores(CacheKey cacheKey, double min, double max, long offset, long count);

    /**
     * 删除指定分数范围内的元素
     */
    Long removeRangeByScore(CacheKey cacheKey, double min, double max);
