package io.hankun.framework.mq.rocketmq.consumer.strategy;

import io.hankun.framework.mq.config.MqProperties;
import io.hankun.framework.mq.constants.EnvMark;
 ...
 */
public class ProdAllocateMessageQueueStrategy implements AllocateMessageQueueStrategy {
    private final InternalLogger log = ClientLogger.getLog();

    private MqProperties mqProperties;

    public ProdAllocateMessageQueueStrategy(MqProperties mqProperties) {
    ...
    public List<MessageQueue> allocate(String consumerGroup, String currentCID, List<MessageQueue> mqAll, List<String> cidAll) {
        ...
        if (!selectedCidAll.contains(currentCID)) {
            log.info("[BUG] ConsumerGroup: {} The consumerId: {} not in selectedCidAll: {}", consumerGroup, currentCID, selectedCidAll);
            return result;
        }
