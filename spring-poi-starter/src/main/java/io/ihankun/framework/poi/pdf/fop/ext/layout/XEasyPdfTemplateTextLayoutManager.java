package io.ihankun.framework.poi.pdf.fop.ext.layout;
...
public class XEasyPdfTemplateTextLayoutManager extends LeafNodeLayoutManager {
     ...
     * logging instance
     */
    private static final Log LOG = LogFactory.getLog(XEasyPdfTemplateTextLayoutManager.class);
    ...
    private final List<Object> changeList = new LinkedList<>();
     ...
     */
    public XEasyPdfTemplateTextLayoutManager(FOText node, FOUserAgent userAgent) {
        foText = node;
    ...
    private final class TextAreaBuilder {
        ...
        private void addWord(int startIndex, int endIndex, int wordLength) {
            ...
            textArea.addWord(wordChars.toString(), wordIPD, letterSpaceAdjust, getNonEmptyLevels(), gposAdjustments, blockProgressionOffset, isWordSpace(endIndex + 1));
        }

        private boolean isWordSpace(int mappingIndex) {
            return userAgent.isAccessibilityEnabled() && mappingIndex < mappings.size() - 1 && getGlyphMapping(mappingIndex).isSpace;
        }
    ...
    public List getNextKnuthElements(final LayoutContext context, final int alignment) {
        ...
        while (nextStart < foText.length()) {
            ...
            if (inWord) {
                ...
                if (mapping != null) {
                    prevMapping = mapping;
                    processLeftoverGlyphMapping(alignment, sequence, mapping, true);
                    mapping = null;
                }
            ...
            if (ch == CharUtilities.SPACE && foText.getWhitespaceTreatment() == Constants.EN_PRESERVE || ch == CharUtilities.NBSPACE) {
                ...
                mapping = new GlyphMapping(nextStart, nextStart + 1, 1, 0, areaIPD, false, true, breakOpportunity, spaceFont, level, null);
    ...
    public List addALetterSpaceTo(final List oldList, int depth) {
        ...
        if (index > -1) {
            ...
            if (BREAK_CHARS.indexOf(foText.charAt(tempStart - 1)) >= 0) {
    ...
    public void hyphenate(Position pos, HyphContext hyphContext) {
        ...
        while (startIndex < mapping.endIndex) {
            ...
            if (!(nothingChanged && stopIndex == mapping.endIndex && !hyphenFollows)) {
                ...
                changeList.add(new PendingChange(new GlyphMapping(startIndex, stopIndex, 0, letterSpaceCount, newIPD, hyphenFollows, false, false, font, -1, null), glyphIndex));
    ...
    private void addElementsForAHyphen(List baseList, int alignment, int widthIfBreakOccurs, MinOptMax widthIfNoBreakOccurs, boolean unflagged) {
        ...
        switch (alignment) {
            case EN_CENTER:
                ...
                baseList.add(new KnuthPenalty(hyphIPD, unflagged ? SOFT_HYPHEN_PENALTY : KnuthPenalty.FLAGGED_PENALTY, !unflagged, auxiliaryPosition, false));
            ...
            case EN_END:
                ...
                if (lineStartBAP != 0 || lineEndBAP != 0) {
                    ...
                    baseList.add(new KnuthPenalty(widthIfBreakOccurs, unflagged ? SOFT_HYPHEN_PENALTY : KnuthPenalty.FLAGGED_PENALTY, !unflagged, auxiliaryPosition, false));
