	...
	public BeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & org.springframework.cglib.beans.BeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & org.springframework.cglib.beans.BeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
